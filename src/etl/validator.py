...
def dq05_opm_crosscheck(df):
    ...
    op_diff = (
        (expected_op - df["operating_profit"]).abs()
        / df["sales"]
    ) * 100
    ...
    opm_diff = (
        expected_opm - df["opm_percentage"]
    ).abs()
...
def dq07_year_format(df):
    ...
    invalid = df[
        ~df["year"].astype(str).str.match(pattern, na=False)
    ]
...
def dq08_ticker_format(df):
    ...
    invalid = df[
        ~df["id"].astype(str).str.fullmatch(r"[A-Z&-]+")
    ]
...
def dq10_fixed_assets(df):
    ...
    invalid = df[
        df["fixed_assets"] > df["total_assets"]
    ]
...
def dq15_liability_total(df):
    ...
    invalid = df[
        (expected - df["total_liabilities"]).abs() > 1
    ]
