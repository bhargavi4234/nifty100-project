...
def calculate_profitability_ratios(row):
    ...
    check_opm_difference(opm, row.get("opm_percentage"))
    ...
    benchmark = roce_benchmark(row.get("broad_sector"))

    # Day 09 calculations

    de_ratio = debt_to_equity(row["borrowings"], row["equity_capital"], row["reserves"])

    leverage_flag = high_leverage_flag(de_ratio, row.get("broad_sector", ""))

    icr = interest_coverage_ratio(
        row["operating_profit"], row.get("other_income", 0), row.get("interest", 0)
    )
    ...
    debt = net_debt(row["borrowings"], row.get("investments", 0))
...
def high_leverage_flag(debt_equity, broad_sector):
    ...
    return debt_equity > 5 and broad_sector.lower() != "financials"
