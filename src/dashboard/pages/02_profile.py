...
if not pl.empty:
    ...
    chart_pl = pl.dropna(
        subset=["year_number"]
    ).sort_values("year_number").tail(10)

    fig = go.Figure()

    fig.add_trace(
        go.Bar(
            x=chart_pl["year_number"],
            y=chart_pl["sales"],
            name="Revenue"
        )
    )
    ...
    st.info(
        "Revenue and Net Profit data not available."
    )
...
if not ratios.empty:
    ...
    st.info(
        "ROE and ROCE data not available."
    )
...
if not pros_cons.empty:
    ...
    st.info(
        "Pros and cons information not available."
    )