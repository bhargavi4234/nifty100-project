...
roe_min_value = numeric_min("return_on_equity_pct")

roe_max_value = numeric_max("return_on_equity_pct")

de_min_value = numeric_min("debt_to_equity")

de_max_value = numeric_max("debt_to_equity")

fcf_min_value = numeric_min("free_cash_flow_cr")

fcf_max_value = numeric_max("free_cash_flow_cr")

revenue_cagr_min_value = numeric_min("revenue_cagr_5yr")

revenue_cagr_max_value = numeric_max("revenue_cagr_5yr")

pat_cagr_min_value = numeric_min("pat_cagr_5yr")

pat_cagr_max_value = numeric_max("pat_cagr_5yr")

opm_min_value = numeric_min("operating_profit_margin_pct")

opm_max_value = numeric_max("operating_profit_margin_pct")

pe_min_value = numeric_min("pe_ratio")

pe_max_value = numeric_max("pe_ratio")

pb_min_value = numeric_min("pb_ratio")

pb_max_value = numeric_max("pb_ratio")

dividend_min_value = numeric_min("dividend_yield_pct")

dividend_max_value = numeric_max("dividend_yield_pct")

icr_min_value = numeric_min("interest_coverage")

icr_max_value = numeric_max("interest_coverage")
...
def apply_preset(preset_name):
    ...
    st.session_state["roe_slider"] = preset.get("roe_min", roe_min_value)

    st.session_state["de_slider"] = preset.get("debt_equity_max", de_max_value)

    st.session_state["fcf_slider"] = preset.get("fcf_min", fcf_min_value)

    st.session_state["revenue_slider"] = preset.get(
        "revenue_cagr_5yr_min", revenue_cagr_min_value
    )

    st.session_state["pat_slider"] = preset.get("pat_cagr_5yr_min", pat_cagr_min_value)

    st.session_state["opm_slider"] = preset.get("opm_min", opm_min_value)

    st.session_state["pe_slider"] = preset.get("pe_max", pe_max_value)

    st.session_state["pb_slider"] = preset.get("pb_max", pb_max_value)

    st.session_state["dividend_slider"] = preset.get(
        "dividend_yield_min", dividend_min_value
    )

    st.session_state["icr_slider"] = preset.get("icr_min", icr_min_value)
...
for i in range(0, len(preset_names), 2):
    ...
    with col1:
        if st.button(label1, key=f"preset_{preset1}", use_container_width=True):
    ...
    if i + 1 < len(preset_names):
        ...
        with col2:
            if st.button(label2, key=f"preset_{preset2}", use_container_width=True):
...
try:
    ...
    st.error(f"Unable to apply screener filters: {e}")
